...
int UA_xc, UA_yc, UA_height, UA_width, UA_Awidth, UA_peak;
UA_xc = 100; UA_yc = 110;
UA_height = 60; UA_width = 40;
UA_Awidth = 20; UA_peak = 30;

void vid_draw_arrow(int x_start, int y_start, int width, int height, int A_width, int A_size, int orientation, int color, alt_video_display* display)
{
	int x[7], y[7]; int k;

	x[0] = x_start; y[0] = y_start;
	if (orientation) {
		x[1] = x_start+width; y[1] = y_start+width;
		x[2] = x[1]+height; y[2] = y[1]-height;
		x[3] = x[2]+A_width; y[3] = y[2]+A_width;
		x[4] = x_start+A_size; y[4] = y_start+width-A_size;
		x[6] = x_start+height; y[6] = y_start-height;
		x[5] = x[6]-A_width; y[5] = y[6]-A_width;
	}else {
		x[1] = x_start-width; y[1] = y_start+width;
		x[2] = x[1]+height; y[2] = y[1]+height;
		x[3] = x[2]-A_width; y[3] = y[2]+A_width;
		x[4] = x_start+A_size-width; y[4] = y_start+A_size;
		x[6] = x_start+height; y[6] = y_start+height;
		x[5] = x[6]+A_width; y[5] = y[6]-A_width;
	}

	vid_draw_line(x[0], y[0], x[6], y[6], 2, color,display);
	for (k=0; k<6; k++) vid_draw_line(x[k], y[k], x[k+1], y[k+1], 2, color,display);



/*	int Ax_start, Ax_end, Ay_start, Ay_end;
	if (orientation==1) // vertical
	{
		Ax_start = x_start-A_width; Ax_end = x_start+width+A_width;
		Ay_start = y_start + height; Ay_end = y_start + height+A_height;
		vid_draw_horiz_line(x_start, x_start+width, y_start, color,display);
		vid_draw_horiz_line(Ax_start, x_start, Ay_start, color,display);
		vid_draw_horiz_line(x_start+width, Ax_end, Ay_start, color,display);
		vid_draw_line(x_start, Ay_start, x_start, y_start, 2, color,display);
		vid_draw_line(x_start+width, Ay_start, x_start+width, y_start, 2, color,display);
		vid_draw_line(x_start+(width/2), Ay_end, Ax_start, Ay_start, 2, color,display);
		vid_draw_line(x_start+(width/2), Ay_end, Ax_end, Ay_start, 2, color,display);
	}
	else
	{
		Ax_start = x_start + height; Ax_end = x_start + height+A_height;
		Ay_start = y_start-A_width; Ay_end = y_start+width+A_width;
		vid_draw_line(x_start, y_start, x_start, y_start+width, 2, color, display);
		vid_draw_line(Ax_start, Ay_start, Ax_start, y_start, 2, color, display);
		vid_draw_line(Ax_start, y_start+width, Ax_start, Ay_end, 2, color, display);
		if (height<0){
			vid_draw_line(Ax_start, y_start, x_start, y_start, 2, color,display);
			vid_draw_line(Ax_start, y_start+width, x_start, y_start+width, 2, color,display);
		} else {
			vid_draw_line(x_start, y_start, Ax_start, y_start, 2, color,display);
			vid_draw_line(x_start, y_start+width, Ax_start, y_start+width, 2, color,display);
		}

		vid_draw_line(Ax_end, y_start+(width/2), Ax_start, Ay_start, 2, color,display);
		vid_draw_line(Ax_end, y_start+(width/2), Ax_start, Ay_end, 2, color,display);
	}
	*/
}
